...
class MyDateTime2 {
public:
    MyDateTime2 (uint32_t t =0);
    MyDateTime2 (uint16_t year, uint8_t month, uint8_t day,
                uint8_t hour =0, uint8_t min =0, uint8_t sec =0);
    MyDateTime2 (const MyDateTime2& copy);
    MyDateTime2 (const char* date, const char* time);
    ...
    MyDateTime2 operator+(const TimeSpan& span);
    MyDateTime2 operator-(const TimeSpan& span);
    TimeSpan operator-(const MyDateTime2& right);
...
public:
    boolean begin(void);
    static void adjust(const MyDateTime2& dt);
    uint8_t isrunning(void);
    static MyDateTime2 now();
...
public:
    boolean begin(void);
    void adjust(const MyDateTime2& dt);
...
class RTC_Millis {
public:
    static void begin(const MyDateTime2& dt) { adjust(dt); }
    static void adjust(const MyDateTime2& dt);
    static MyDateTime2 now();
