...
public:
    MyDateTime (long t =0);
    MyDateTime (uint16_t year, uint8_t month, uint8_t date,
    ...
    uint8_t DdayOfWeek() const   { return wday;}  /*Su=1 Mo=2 Tu=3 We=4 Th=5 Fr=6 Sa=7 */
...
public:
    ...
    void setMyDateTime(const MyDateTime& dt);  //Changes the date-time
    void setEpoch(uint32_t ts); // Set the RTC using timestamp (seconds since epoch)
    MyDateTime now();            //Gets the current date-time

    MyDateTime makeDateTime(unsigned long t);
