...
static void hw_initConfigBlock( size_t length = 1024 /*ATMega328 has 1024 bytes*/ )
{
  ...
  if (!initDone)
  {
    ...
    configBlock = (char *)malloc(length);
    if (configBlock != NULL)
        memset(configBlock, 0xff, length);
    initDone = (configBlock != NULL);
  }
}
...
void MyHwESP8266::debugPrint(bool isGW, const char *fmt, ... ) {
	...
	if (isGW) {
		...
		Serial.printf("0;0;%d;0;%d;", C_INTERNAL, I_LOG_MESSAGE);
	}
	va_list args;
	va_start (args, fmt );
	va_end (args);
